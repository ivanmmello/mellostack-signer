package org.icpbrasil.signer.providers.birdid;

import org.icpbrasil.signer.model.Environment;
import org.icpbrasil.signer.provider.PSCProvider;
import org.icpbrasil.signer.providers.http.HttpClientConfig;
import org.icpbrasil.signer.providers.http.SecureHttpClient;
import org.icpbrasil.signer.providers.security.SecureTokenCache;

import java.util.Objects;

/**
 * Builder para {@link BirdIdProvider}.
 * <p>
 * Credenciais devem ser obtidas de variáveis de ambiente ou cofre seguro —
 * nunca commitar {@code clientSecret} no repositório.
 */
public final class BirdIdProviderBuilder {

    private String clientId;
    private String clientSecret;
    private Environment environment = Environment.HOMOLOGATION;
    private String certificateAlias;
    private HttpClientConfig httpClientConfig = HttpClientConfig.defaults();

    public BirdIdProviderBuilder withClientId(String clientId) {
        this.clientId = clientId;
        return this;
    }

    public BirdIdProviderBuilder withClientSecret(String clientSecret) {
        this.clientSecret = clientSecret;
        return this;
    }

    public BirdIdProviderBuilder withEnvironment(Environment environment) {
        this.environment = Objects.requireNonNull(environment, "environment");
        return this;
    }

    /**
     * Alias do certificado Bird ID. Obrigatório quando o usuário possui múltiplos certificados.
     */
    public BirdIdProviderBuilder withCertificateAlias(String certificateAlias) {
        this.certificateAlias = certificateAlias;
        return this;
    }

    public BirdIdProviderBuilder withHttpClientConfig(HttpClientConfig httpClientConfig) {
        this.httpClientConfig = Objects.requireNonNull(httpClientConfig, "httpClientConfig");
        return this;
    }

    public PSCProvider build() {
        validateCredentials(clientId, "clientId");
        validateCredentials(clientSecret, "clientSecret");
        if (certificateAlias != null && !certificateAlias.isBlank()) {
            BirdIdApiClient.validateCertificateAlias(certificateAlias);
        }

        SecureHttpClient httpClient = new SecureHttpClient(httpClientConfig);
        BirdIdApiClient apiClient = new BirdIdApiClient(environment, httpClient);
        BirdIdOAuth2Support oauth2Support = new BirdIdOAuth2Support(
                environment, clientId, clientSecret, httpClient);
        SecureTokenCache<BirdIdApiClient.BirdIdCertificate> cache =
                new SecureTokenCache<>(BirdIdProvider.CERTIFICATE_CACHE_TTL);

        return new BirdIdProvider(
                environment,
                apiClient,
                oauth2Support,
                certificateAlias,
                cache
        );
    }

    private static void validateCredentials(String value, String fieldName) {
        if (value == null || value.isBlank()) {
            throw new IllegalArgumentException(fieldName + " must not be blank");
        }
        if (value.length() > 512) {
            throw new IllegalArgumentException(fieldName + " exceeds maximum allowed length");
        }
    }
}
