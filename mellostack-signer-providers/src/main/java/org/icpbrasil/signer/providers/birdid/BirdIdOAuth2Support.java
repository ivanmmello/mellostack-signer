...
/**
 * Utilitários OAuth2 Bird ID para a aplicação host (Authorization Code + PKCE).
 */
public final class BirdIdOAuth2Support {
    ...
    BirdIdOAuth2Support(
        ...
        this.delegate = new ItiCloudPscOAuth2Support(
                new ItiCloudPscEndpoints(BirdIdEndpoints.baseUrl(environment)),
                clientId,
                clientSecret,
                httpClient
        );
    ...
    static BirdIdOAuth2Support create(
            Environment environment,
            String clientId,
            String clientSecret,
            HttpClientConfig httpClientConfig) {
        SecureHttpClient httpClient = new SecureHttpClient(httpClientConfig);
        return new BirdIdOAuth2Support(environment, clientId, clientSecret, httpClient);
    }
}
